...
import { LayoutDashboard, FileText, ChevronDown, LogOut, User as UserIcon, Sun, Moon, Palette, Monitor, Shield, Info, UploadCloud } from "lucide-react";
...
export function Navbar({ user }) {
  ...
  useEffect(() => {
    function handleClickOutside(event) {
      if (pcapRef.current && !pcapRef.current.contains(event.target)) setPcapMenuOpen(false);
    ...
    <>
      <nav className="bg-nav border-b border-theme h-14 flex items-center px-6 gap-8 sticky top-0 z-50 transition-colors">
        ...
        <div className="flex items-center flex-1 h-full">
          {isAdmin ? (
            <>
              <NavLink href="/dashboard" icon={LayoutDashboard} label="Dashboard" isActive={pathname === "/dashboard"} />
              ...
              >
                ...
                >
                  <FileText size={16} className={pathname.startsWith("/pcaps") ? "text-blue-500" : "text-slate-400"} />
                  PCAPS
                  <ChevronDown size={14} className={`transition-transform duration-200 ${pcapMenuOpen ? "rotate-180" : ""}`} />
                  {pathname.startsWith("/pcaps") && (
                    <span className="absolute bottom-0 left-0 w-full h-[2px] bg-blue-500 rounded-t-md shadow-[0_0_8px_rgba(59,130,246,0.5)]" />
                  )}
                </button>
              ...
              <NavLink href="/upload" icon={UploadCloud} label="Upload" isActive={pathname === "/upload"} />
            ...
            <>
              ...
              >
                ...
                >
                  <FileText size={16} className={pathname.startsWith("/reports") ? "text-blue-500" : "text-slate-400"} />
                  Reports
                  <ChevronDown size={14} className={`transition-transform duration-200 ${pcapMenuOpen ? "rotate-180" : ""}`} />
                  {pathname.startsWith("/reports") && (
                    <span className="absolute bottom-0 left-0 w-full h-[2px] bg-blue-500 rounded-t-md shadow-[0_0_8px_rgba(59,130,246,0.5)]" />
          ...
          {userMenuOpen && (
            <div className="absolute top-14 right-0 bg-card border border-theme rounded-xl shadow-2xl min-w-[220px] p-1.5 z-50 animate-in fade-in slide-in-from-top-2 duration-200">
              <div className="px-3 py-2 border-b border-theme mb-1">
                <div className="text-[13px] font-black text-foreground">{user?.name || "Logged In"}</div>
              ...
              <button 
                ...
                className="w-full text-left px-3 py-2 text-[13px] font-bold text-slate-500 rounded-md hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors flex items-center gap-2"
              >
