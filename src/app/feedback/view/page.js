...
export default function ViewFeedbackPage() {
   ...
   * /feedback/view is admin-only.
   * Normal user → /feedback/view → /reports?set=1
   * Admin       → /feedback/view → fetch feedback
   */
  useEffect(() => {
  ...
  // While NextAuth is checking the session, don't show the feedback page.
  if (status === "loading") {
    return (
      <div className="min-h-screen flex items-center justify-center bg-card">
        <div className="text-center">
          <div className="inline-block animate-spin rounded-full h-8 w-8 border-b-2 border-blue-500 mb-3" />
          <p className="text-sm font-serif">Checking permissions...</p>
        </div>
      </div>
    );
  }

  // If user is not admin, redirect is being handled by the useEffect. Don't render the feedback content.
  const roles = session?.user?.roles || [];
  const isAdmin = roles.some((role) => role?.toLowerCase() === "admin");
  ...
  if (status !== "authenticated" || !isAdmin) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-card">
        <div className="text-center">
          <p className="text-sm font-serif text-slate-500">Redirecting...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-card dark:bg-slate-900">
      <div className="w-full min-h-screen flex flex-col bg-card">
        {/* Header */}
